...
Today:
- The cache-refresh path (`fetchBatch` in the bundled API) **already fetches `listings=LISTINGS_CAP=50`** — so the raw rows exist at fetch time.
- `parseMarketResponse` keeps `WorldListing = { world, price, hq }` — **quantity and retainer/seller name are dropped** — and `LISTINGS_KEPT = 10` truncates to the cheapest 10 rows. The full book is otherwise represented solely by the scalar `listingCount`.
- The web app reads the resulting `market-cache.json` (via `loadSharedMarketCache`), **not** `buildMarketUrl` (which only feeds the live bot-chat fallback). So the parser is the *only* truncation point for the web feature.

This makes a true depth chart and a true seller-HHI impossible. The fix is one contained change to `parseMarketResponse`:

- `RawListing`: also read `quantity` and `retainerName`.
- `WorldListing`: add **optional** `quantity?: number` and `seller?: string`. Optional (not required) because ~54 files construct `worldListings` literals — optional fields keep that blast radius at zero; compute functions default `quantity ?? 1` / treat missing `seller` as absent.
- `LISTINGS_KEPT`: `10` → `LISTINGS_CAP` (50) so the full book (up to the cap, already fetched) is cached for both views.
- `buildMarketUrl`'s `listings=10` (live bot-chat path only) is **left unchanged** — out of scope.
- After the source change, `npm run build:api` regenerates the `api/*.mjs` bundles so the deployed cache refresh emits the enriched rows.
...
**Modify:**
- `src/lib/universalis.ts` — `RawListing`, `WorldListing`, parser, `LISTINGS_KEPT`.
- `src/lib/universalis.test.ts` — update parser expectations for the new fields + the kept-rows count.
- `src/routes/Item.tsx` — mount the two new blocks (no change to the verdict pipeline).
- `api/discord.mjs`, `api/refresh-cache.mjs` (+ any other `api/*.mjs` importing universalis) — regenerated by `npm run build:api`.

**Add:**
- `src/features/items/depth.ts` (+ `.test.ts`) — `depthBuckets`.
- `src/features/items/concentration.ts` (+ `.test.ts`) — `concentrationHHI`, `RiskLevel`.
- `src/features/items/QualityTab.tsx` — shared NQ/HQ toggle button (used by both blocks).
