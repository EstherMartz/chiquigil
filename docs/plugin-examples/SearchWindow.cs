...
/// <summary>
/// Search window for browsing items and their sources.
/// Displays search results in a table and shows detailed sources when an item is selected.
/// </summary>
public class SearchWindow : Window, IDisposable
{
    private readonly PluginApiClient _apiClient;
    ...
    public SearchWindow(PluginApiClient apiClient) : base("Item Search")
    {
    ...
    public override void Draw()
    {
        ...
        if (_isSearching)
        {
            ImGui.Spinner("##searching", 15, 4, ImGui.GetColorU32(ImGuiCol.ButtonHovered));
            ImGui.SameLine();
        ...
        if (ImGui.BeginTable("##itemsTable", 3, ImGuiTableFlags.Borders | ImGuiTableFlags.RowBg | ImGuiTableFlags.ScrollY))
        {
            ...
            foreach (var item in _searchResults)
            {
                ...
                if (item.Rarity > 0)
                {
                    ...
                    {
                        1 => new Vector4(0.7f, 0.7f, 0.7f, 1), // Common (gray)
                        2 => new Vector4(0.2f, 0.8f, 0.2f, 1), // Uncommon (green)
                        3 => new Vector4(0.2f, 0.6f, 1, 1),    // Rare (blue)
                        4 => new Vector4(1, 0.5f, 0, 1),       // Epic (orange)
                        _ => new Vector4(1, 1, 1, 1),          // Unknown (white)
                    };
    ...
    private void DrawSourcesModal()
    {
        ...
        if (ImGui.BeginPopupModal($"Sources: {_selectedItem.Name}##sources", ref (_selectedItem != null)))
        {
            ...
            {
                ImGui.Spinner("##loadingSources", 15, 4, ImGui.GetColorU32(ImGuiCol.ButtonHovered));
                ImGui.SameLine();
    ...
    private void DrawRecipeSource(RecipeSource recipe)
    {
        ...
        ImGui.TextColored(headerColor, $"📖 {recipe.JobName} (Lv. {recipe.Level})");
        ...
        {
            ImGui.BulletText($"{ingredient.ItemName} x{ingredient.Qty}");
        }
        ImGui.Unindent();
    }
    ...
    private void DrawVendorSource(VendorSource vendor)
    {
        ...
        ImGui.TextColored(headerColor, $"🏪 NPC Vendor");
    ...
    private void DrawGatheringSource(GatheringSource gathering)
    {
        ...
        ImGui.TextColored(headerColor, $"⛏️ Gathering (Lv. {gathering.Level}){timedLabel}");
    }

    private void DrawSpecialShopSource(SpecialShopSource specialShop)
    {
        var headerColor = new Vector4(1, 0.5f, 0.8f, 1); // Pink for special shops
        ImGui.TextColored(headerColor, $"⭐ Special Shop");
    ...
    {
        var headerColor = new Vector4(1, 0.7f, 0.2f, 1); // Orange for company crafts
        ImGui.TextColored(headerColor, $"🏢 {companyCraft.CraftName}");

        ImGui.Indent();
        ImGui.TextColored(new Vector4(0.8f, 0.8f, 0.8f, 1), "Materials:");
        foreach (var ingredient in companyCraft.Ingredients)
        {
            ImGui.BulletText($"{ingredient.ItemName} x{ingredient.Qty}");
        }
        ImGui.Unindent();
    ...
    private async Task PerformSearch()
    {
        ...
        try
        {
            var response = await _apiClient.SearchItems(_searchQuery, _currentPage, PageSize);
    ...
    private async Task LoadItemSources(int itemId)
    {
        try
        {
            var response = await _apiClient.GetItemSources(itemId);
            _selectedSources = response;
            _sourcesError = null;
        }
        catch (Exception ex)
        {
