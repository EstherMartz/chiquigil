...
const KIND_LABEL: Record<IngredientSourceKind, string> = {
  gather: 'Gather', craft: 'Craft', buy: 'Buy',
};
...
export function IngredientBreakdownModal({
  itemId, itemName, recipe, recipeMap, market, gatherableIds, nameOf, onClose,
}: {
  itemId: number;
  itemName: string;
  recipe: Recipe;
  ...
  const rows = selfSourceBreakdown(recipe, recipeMap, market, gatherableIds);
  const total = rows.reduce((s, r) => s + r.lineCost, 0);

  function planCraft() {
    addItem(itemId, 1);
    setCraftIntermediates(itemId, true); // self-source plan = explode sub-recipes
    onClose();
  }
    ...
    >
      ...
      >
        ...
        <p className="font-mono text-[10px] text-text-low mt-3">
          <span className="text-aether">Gather</span> mats cost time, not gil. <span className="text-gold">Craft</span> mats
          recurse into their own cost. <span className="text-text-dim">Buy</span> mats use the home-world price.
        </p>
